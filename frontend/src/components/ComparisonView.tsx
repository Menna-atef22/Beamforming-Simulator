...
export default function ComparisonView({ before, after, title = "Before vs After" }: ComparisonViewProps) {
  const data = before.angles
    ...
    .map((angle, i) => ({
      angle,
      before: before.magnitudes[i * 2],
      after: after.magnitudes[i * 2],
    }));
      ...
      <div className="flex-1 min-h-0">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 5, right: 10, bottom: 20, left: 10 }}>
            ...
            <Line type="monotone" dataKey="before" stroke="hsl(240,50%,55%)" strokeWidth={1.5} dot={false} name="No Steering" strokeDasharray="5 5" />
            <Line type="monotone" dataKey="after" stroke="hsl(320,70%,60%)" strokeWidth={2} dot={false} name="Steered + Processed" />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
