import { BeamformingParams, BeamMetrics, WindowType } from "@/types/beamforming";
...
interface ControlPanelProps {
  ...
  onParamChange: <K extends keyof BeamformingParams>(key: K, value: BeamformingParams[K]) => void;
  metrics?: BeamMetrics;
...
function SectionHeader({ title }: { title: string }) {
  return (
    <div className="flex items-center gap-2 pt-3 pb-1">
      ...
      <h3 className="text-[10px] font-mono font-bold uppercase tracking-widest text-muted-foreground">{title}</h3>
    </div>
  );
}
  ...
  return (
    <div className="p-4 space-y-3 h-full overflow-y-auto bg-card/60">
      ...
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-xs font-mono font-bold uppercase tracking-widest text-foreground">Parameters</h2>
      </div>
      ...
      <SliderControl label="SNR" value={params.snrDb} min={0} max={60} step={1} unit="dB" onChange={(v) => onParamChange("snrDb", v)} />
      <div className="flex justify-between items-center py-1">
        <Label className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider">Phase Shift</Label>
        <span className="text-xs font-mono text-foreground tabular-nums">{phaseShift} rad</span>
      </div>

      {/* Windowing */}
      <SectionHeader title="Windowing" />
      <Select value={params.windowType} onValueChange={(v) => onParamChange("windowType", v as WindowType)}>
      ...
      <div className="flex justify-between items-center py-1">
        <Label className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider">Noise</Label>
        <Switch checked={params.noiseEnabled} onCheckedChange={(v) => onParamChange("noiseEnabled", v)} />
      </div>
      <div className="flex justify-between items-center py-1">
        <Label className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider">Apodization</Label>
        <Switch checked={params.apodizationEnabled} onCheckedChange={(v) => onParamChange("apodizationEnabled", v)} />
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-secondary/50 rounded-lg p-2 text-center">
              <div className="text-[9px] font-mono text-muted-foreground uppercase">Beamwidth</div>
              <div className="text-sm font-mono font-bold text-foreground">{metrics.beamwidthDeg.toFixed(1)}°</div>
            </div>
            <div className="bg-secondary/50 rounded-lg p-2 text-center">
              <div className="text-[9px] font-mono text-muted-foreground uppercase">SLL</div>
              <div className="text-sm font-mono font-bold text-foreground">{isFinite(metrics.sllDb) ? metrics.sllDb.toFixed(1) : "—"} dB</div>
            </div>
          </div>
        </>
      )}
